...
def collect_r2c_data(data_root, mode='train', record_file='./data/refsplits.json'):
    ...
    # assert os.path.exists(camo_Imgs_dir) and os.path.exists(camo_gts_dir)  # 没有这个文件
    ...
    ref_images_dir = os.path.join(data_root, 'Ref', 'Images')
    assert os.path.exists(ref_images_dir)
    ref_classes = os.listdir(ref_images_dir) # 49个类别
    ...
    # image_label_list = []
    class_file_list = {}
    for c_idx in tqdm(range(len(ref_classes))):
        cate = ref_classes[c_idx]
        ...
        # image_label_list += [(os.path.join(camo_cate_Imgs_dir, camo_img_names[f_idx]),
        #                       os.path.join(camo_cate_gts_dir, camo_gt_names[f_idx])) for f_idx in
        #                      range(len(camo_img_names))]

        # ref_cate_feats_dir = os.path.join(ref_feats_dir, cate)
        ref_cate_images_dir = os.path.join(ref_images_dir, cate)

        ref_cate_split_names = splits[mode if mode != 'val' else 'test'][cate]
        # class_file_list[cate] = [os.path.join(ref_cate_feats_dir, ref_cate_split_names[f_idx] + '.npy') for f_idx in
        #                          range(len(ref_cate_split_names))]
        class_file_list[cate] = [os.path.join(ref_cate_images_dir, ref_cate_split_names[f_idx] + '.jpg') for f_idx in
                                 range(len(ref_cate_split_names))]

    # print('>>> {}ing with {} r2c samples'.format(mode, len(image_label_list)))
    print('>>> {}ing with {} r2c samples'.format(mode, len(class_file_list)))

    return class_file_list

# def collect_r2c_data(data_root, mode='train', record_file='./data/refsplits.json'): # 源代码
#
#     if not os.path.exists(record_file):
#         split_ref_data(data_root, record_file)
#
