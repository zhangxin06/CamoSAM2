...
class Hiera(nn.Module):
    ...
    def __init__(
        ...
        self.embed_dim = [96, 192, 384, 768]  # sam2_hiera_s
        # self.embed_dim = [144, 288, 576, 1152]  # sam2_hiera_l
        self.depth = stages
    ...
    # def forward(self, x, index):
        inp = x
        x = self.patch_embed(x)  # (1,3,1024,1024)->(1,256,256,96)
        # Add pos embed
        ...
        # x: (B, H, W, C)
        # handcrafted1, handcrafted2, handcrafted3, handcrafted4 = self.prompt_generator.init_handcrafted(inp)  # fft
        #
        # self.block1, self.block2, self.block3, self.block4 = [],[],[],[]
        # outputs = []  # sam2_hiera_l
        # for i, blk in enumerate(self.blocks):
        #     if i < 3:
        ...
        # handcrafted1, handcrafted2, handcrafted3, handcrafted4 = self.prompt_generator.init_handcrafted(inp)  # fft
        # self.block1, self.block2, self.block3, self.block4 = [],[],[],[]
        # for i, blk in enumerate(self.blocks): # sam2_hiera_s
        #     if i < 2:
        ...
        #     elif 14 < i:
        #         self.block4.append(blk)  # 第四个块包含接下来剩余的1个元素
        #
        # outputs = []
        # if '1' in self.tuning_stage: # '1234'
        #     prompt1 = self.prompt_generator.init_prompt(x, handcrafted1, 1)
        # for i, blk in enumerate(self.block1):
        #     if '1' in self.tuning_stage:
        #         x = self.prompt_generator.get_prompt(x, prompt1, 1, i)
