...
class Decoder_zx(nn.Module):
    ...
    def forward(self, zt5, zt4, zt3, zt2):
        zt5_1 = zt5
...
class Motion_guided_decoder(nn.Module):
    def __init__(self, channel):
        super(Motion_guided_decoder, self).__init__()
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv_upsample1 = ConvBR(channel, channel, 3, padding=1)
        self.conv_upsample2 = ConvBR(channel, channel, 3, padding=1)
        self.conv_upsample3 = ConvBR(channel, channel, 3, padding=1)
        ...
        self.conv_concat_corr3 = ConvBR(channel*2, channel*2, 3, padding=1)
        self.conv_downsample3 = ConvBR(channel*2, channel*2, 3, padding=1)
        self.conv_concat_corr4 = ConvBR(channel*2, channel*2, 3, padding=1)
        self.conv_concat_corr4_1 = ConvBR(channel*4, channel*4, 3, padding=1)
        self.conv_concat_corr5 = ConvBR(channel*2, channel*2, 3, padding=1)
        self.conv_concat_corr5_1 = ConvBR(channel*6, channel*6, 3, padding=1)
        self.conv_downsample4 = ConvBR(channel*4, channel*4, 3, padding=1)
        self.conv_back_zt3 = ConvBR(channel*2,channel,3,padding=1)
        self.conv_back_zt4 = ConvBR(channel*4,channel,3,padding=1)
        self.conv_back_zt5 = ConvBR(channel*6,channel,3,padding=1)

    def forward(self, zt5, zt4, zt3, corr):
        corr_zt3 = corr
        corr_zt4 = F.interpolate(corr, scale_factor=1/2, mode='bilinear')
        corr_zt5 = F.interpolate(corr, scale_factor=1/4, mode='bilinear')

        zt3 = self.conv_concat_corr3(torch.cat((zt3, zt3*corr_zt3),1))
        zt4 = self.conv_concat_corr4(torch.cat((zt4, zt4*corr_zt4),1))
        zt4 = self.conv_concat_corr4_1(torch.cat((zt4, self.conv_downsample3(self.downsample(zt3))),1))
        zt5 = self.conv_concat_corr5(torch.cat((zt5, zt5*corr_zt5),1))
        zt5 = self.conv_concat_corr5_1(torch.cat((zt5, self.conv_downsample4(self.downsample(zt4))),1))

        zt3 = self.conv_back_zt3(zt3)
        zt4 = self.conv_back_zt4(zt4)
        zt5 = self.conv_back_zt5(zt5)
        ...
        res = F.interpolate(pc, scale_factor=8, mode='bilinear') # (bs,1,352,352)
        return res
...
class FeatureExtraction(nn.Module):
    def __init__(self, channel=32, pretrained=None, backbone_name='efficientnet_b4', input_shape=None):
        ...
        if backbone_name == 'res2net50_26w_4s':
            ...
            # self.resnet50_en = models.resnet50(weights=torchvision.models.ResNet50_Weights.IMAGENET1K_V1)
            self.resnet50_en = models.resnet50(pretrained=True)
        else:
        ...
        if self.pretrained:
            if backbone_name == 'pvt_v2_b5':
                ...
                self.load_model_resnet(self.pretrained)
            else:
                pass
    ...
    def load_model(self, pretrained):
        ...
        for k, v in pretrained_dict.items():
            if k.startswith('backbone.'):
                k = k.replace('backbone.', 'pvtv2_en.') # pvtv2_en
            # pdb.set_trace()
            if (k in model_dict):
                print("load:%s" % k)
            else:
    ...
    def load_model_resnet(self, pretrained):
        pretrained_dict = torch.load(pretrained)['state_dict']
        model_dict = self.state_dict()
        print("Load pretrained parameters from {}".format(pretrained))
        for k, v in pretrained_dict.items():
            if k.startswith('backbone.'):
                k = k.replace('backbone.', 'resnet50_en.')
            # pdb.set_trace()
            if (k in model_dict):
                print("load:%s" % k)
            else:
        ...
        pretrained_dict = {k.replace('backbone.', 'resnet50_en.'): v for k, v in pretrained_dict.items() if (k.replace('backbone.', 'resnet50_en.') in model_dict)}
        model_dict.update(pretrained_dict)
        self.load_state_dict(model_dict)
    ...
    def forward(self, x):
        ...
        if self.backbone_name == 'res2net50_26w_4s':  
            ...
            x = self.resnet50_en.conv1(x)
            x = self.resnet50_en.bn1(x)
            x = self.resnet50_en.relu(x)
            x = self.resnet50_en.maxpool(x)  
            x1 = self.resnet50_en.layer1(x)  
            x2 = self.resnet50_en.layer2(x1)  
            x3 = self.resnet50_en.layer3(x2)  
            x4 = self.resnet50_en.layer4(x3) 
