...
class CamObjDataset(data.Dataset):
    def __init__(self, root, trainsize, clip_len=5):
        ...
        for vid in vid_list:
            # vid_path = os.path.join(image_root, vid, "Frame")
            vid_path = os.path.join(image_root, vid, "Imgs")
            if 'Kvasir' not in vid:
                # frms = sorted(os.listdir(vid_path), key=lambda x: int(x[:-4]))
                frms = sorted(os.listdir(vid_path))
            else:
            ...
            for idx in range(len(frms)):
                ...
                self.images.append(clip)
                # self.gts.append([x.replace("Frame", "GT").replace("jpg", "png") for x in clip])
                self.gts.append([x.replace("Imgs", "GT").replace("jpg", "png") for x in clip])
...
class CamObjTestDataset(data.Dataset):
    def __init__(self, root, trainsize, clip_len=5):
        self.trainsize = trainsize
        ...
        for vid in vid_list:
            ...
            for idx in range(len(frms)):
                clip = []
                for ii in range(-clip_len // 2 + 1, clip_len // 2 + 1): # range(-2,3): -2,-1,0,1,2 ; -5//2=-3;“//”运算符在Python中是执行地板除法（向下取整）的运算符
                    # pick_idx = idx + ii if idx - ii < 0 else idx - ii
                    pick_idx = idx + ii
                    if pick_idx >= len(frms):
                        pick_idx = len(frms) - 1
    ...
    def __getitem__(self, index):
        ...
        # 2024-6-8 zhangxin
        # gt_path = self.gts[index][self.clip_len // 2]
        # video_name = gt_path.split('/GT')[0].split('/')[-1]
        ...
        # return torch.stack(images), gt, video_name, frame_name
...
def get_trainloader(image_root, batchsize, trainsize, clip_len=5, shuffle=True, num_workers=12, pin_memory=True):
    ...
    dataset = CamObjDataset(image_root, trainsize, clip_len)
...
#
#     dataset = CamObjTestDataset(image_root, trainsize, clip_len)
#     data_loader = data.DataLoader(dataset=dataset,
#                                   batch_size=batchsize,
#                                   shuffle=shuffle,
#                                   num_workers=num_workers,
...
class test_dataset:
    def __init__(self, images_root, gts_root, testsize, clip_len=5):
        self.testsize = testsize

        self.gt_list = []
        vid_list = os.listdir(images_root)
        vid_list = sorted(vid_list)
        self.clip_len = clip_len
        self.images = []
        self.gts = []
        ...
        for vid in vid_list:
            vid_path = os.path.join(images_root, vid, 'Imgs')
            frms = sorted(os.listdir(vid_path))
            # print(frms)
            for idx in range(len(frms)):
                clip = []
                for ii in range(-clip_len // 2 + 1,
                                clip_len // 2 + 1):  # range(-2,3): -2,-1,0,1,2 ; -5//2=-3;“//”运算符在Python中是执行地板除法（向下取整）的运算符
                    # pick_idx = idx + ii if idx - ii < 0 else idx - ii
                    pick_idx = idx + ii
                    if pick_idx >= len(frms):
                        pick_idx = len(frms) - 1
    ...
    def load_data(self):
        ...
        gt = self.binary_loader(self.gts[self.index][self.clip_len // 2])

        gt = np.array(gt)
        # gt[gt!=0]=255
        gt = Image.fromarray(gt)
        # gt = self.gt_transform(gt)
        gt_tensors = self.gt_transform(gt)
        ...
        return torch.stack(images), gt, gt_tensors
...
def get_trainloader_image(image_root, batchsize, trainsize, clip_len=5, shuffle=True, num_workers=12, pin_memory=True):
...
def get_testloader_image(image_root, batchsize, trainsize, clip_len=5, shuffle=False, num_workers=12, pin_memory=True):
