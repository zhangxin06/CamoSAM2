...
def get_network(args, net, use_gpu=True, gpu_device = 0, distribution = True):
    ...
    if net == 'sam2':
        ...
        # net = build_sam2_video_predictor(config_file=model_cfg, ckpt_path=sam2_checkpoint, mode=None) # 原始
        net = build_sam2_video_predictor(config_file=model_cfg, ckpt_path=sam2_checkpoint, device=gpu_device, mode=None)
    else:
...
# 通过opencv封装函数通过中心矩来找中心点坐标
import cv2
def central_moment(mask, point_labels=1):
    mask = mask.astype(np.uint8) # 和opencv读进来二值化后的类型对应
    contours, _ = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # 找到轮廓
    list_ordinate = []
    for contour in contours:  # 计算每个轮廓的矩
